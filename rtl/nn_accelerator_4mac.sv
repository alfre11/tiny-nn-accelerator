module nn_accelerator_4mac #(
    ...
    initial begin
        if ((INPUT_SIZE % LANES) != 0 || (HIDDEN_SIZE % LANES) != 0) begin
            $fatal(1, "INPUT_SIZE and HIDDEN_SIZE must be multiples of four");
        end
    ...
    logic signed [ACC_WIDTH-1:0] mac_input0;
    logic signed [ACC_WIDTH-1:0] mac_input1;
    logic signed [ACC_WIDTH-1:0] mac_input2;
    logic signed [ACC_WIDTH-1:0] mac_input3;
    logic signed [WEIGHT_WIDTH-1:0] mac_weight0;
    logic signed [WEIGHT_WIDTH-1:0] mac_weight1;
    logic signed [WEIGHT_WIDTH-1:0] mac_weight2;
    logic signed [WEIGHT_WIDTH-1:0] mac_weight3;
    ...
    always_comb begin
        ...
        case (state)
            L1_MAC: begin
                mac_input0 = $signed(input_mem[input_idx]);
                mac_input1 = $signed(input_mem[input_idx + 1]);
                mac_input2 = $signed(input_mem[input_idx + 2]);
                mac_input3 = $signed(input_mem[input_idx + 3]);
                mac_weight0 = weights_l1[hidden_idx*INPUT_SIZE + input_idx];
                mac_weight1 = weights_l1[hidden_idx*INPUT_SIZE + input_idx + 1];
                mac_weight2 = weights_l1[hidden_idx*INPUT_SIZE + input_idx + 2];
                mac_weight3 = weights_l1[hidden_idx*INPUT_SIZE + input_idx + 3];
            end

            L2_MAC: begin
                mac_input0 = hidden_mem[l2_hidden_idx];
                mac_input1 = hidden_mem[l2_hidden_idx + 1];
                mac_input2 = hidden_mem[l2_hidden_idx + 2];
                mac_input3 = hidden_mem[l2_hidden_idx + 3];
                mac_weight0 = weights_l2[output_idx*HIDDEN_SIZE + l2_hidden_idx];
                mac_weight1 = weights_l2[output_idx*HIDDEN_SIZE + l2_hidden_idx + 1];
                mac_weight2 = weights_l2[output_idx*HIDDEN_SIZE + l2_hidden_idx + 2];
                mac_weight3 = weights_l2[output_idx*HIDDEN_SIZE + l2_hidden_idx + 3];
            end
        ...
        if (rst) begin
            ...
            case (state)
                ...
                L1_MAC: begin
                    ...
                    if (input_idx == INPUT_SIZE - LANES) begin
                        state <= L1_STORE;
                    end else begin
                        input_idx <= input_idx + LANES;
                    end
                end
                ...
                L2_MAC: begin
                    ...
                    if (l2_hidden_idx == HIDDEN_SIZE - LANES) begin
                        state <= L2_STORE;
                    end else begin
                        l2_hidden_idx <= l2_hidden_idx + LANES;
                    end
                end
