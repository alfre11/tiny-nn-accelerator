...
module nn_accelerator_4mac_tb;
    ...
    initial begin
        if (NUM_TESTS <= 0) begin
            $fatal(1, "NUM_TESTS must be greater than zero");
        end
        ...
        for (int sample = 0; sample < NUM_TESTS; sample++) begin
            ...
            // Reset between samples, then load the next sample while reset is
            // asserted so inference cannot start with partially updated data.
            @(negedge clk);
            rst = 1'b1;
            start = 1'b0;
        ...
        $display("");
        $display("4-MAC samples tested: %0d", NUM_TESTS);
        ...
        if (total_errors == 0) begin
            $display("4-MAC OVERALL RESULT: PASS");
            $finish;
        end else begin
            $fatal(1, "4-MAC OVERALL RESULT: FAIL");
        end
    end
